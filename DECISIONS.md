...
## 15. Admin data layer is in-memory for this phase

`components/admin/AdminDraftProvider.tsx` seeds React state from the real
`/data` modules (`structuredClone`d so imports are never mutated) and holds all
edits there.

**Nothing persists.** The full editing flow works — edit, dirty tracking per
section, discard, publish — but "Publish changes" only clears the dirty flag,
and a refresh restores the published values. This is intentional for the
validation phase, not a bug. The UI says so: the unpublished-changes bar carries
a "Preview only" tag.

Choosing the real backend (file write to `/data`, a database, or a CMS) is a
separate decision. Whatever it is, it only has to satisfy the same
`HomepageContent` / `Product` types — the provider's consumer contract
(`useDraft()`) can stay as-is.
